...
export default function Game() {
  ...
  const [feedback, setFeedback] = useState(
    "Select a tile, then choose an open square on the board.",
  );
  ...
  const placeTile = (row: number, col: number, rackId = selectedRackId) => {
    ...
    if (board[key] || preview[key]) {
      setFeedback("That square is already occupied. Choose an open square.");
      return;
    }
    if (!rackId) {
  ...
  const submitMove = () => {
    ...
    if (!placed) {
      setFeedback("Place at least one tile before submitting.");
      return;
    }
    ...
    setFeedback(
      `Move submitted locally for ${placed * 8} points. Your turn continues in this preview.`,
    );
  };
  ...
  const showSuggestion = (index: number) => {
    ...
    setPreview(next);
    setFeedback(
      `${suggestion.word} is previewed in violet. Apply it or keep playing manually.`,
    );
  };
  ...
  const confirmExchange = () => {
    ...
    setCurrentRack((current) => [
      ...current.filter((tile) => !exchangeSelection.includes(tile.id)),
      ...replacement,
    ]);
    setExchangeSelection([]);
    setExchangeOpen(false);
    setFeedback(
      `${replacement.length} tile${replacement.length === 1 ? "" : "s"} exchanged locally. The bag count has updated.`,
    );
  };
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
        <main className="min-w-0 space-y-4">
          ...
          <section className="rounded-2xl border border-border bg-card p-3 shadow-sm md:p-5">
            ...
            <div className="mb-4 flex items-start gap-2 rounded-xl border border-primary/15 bg-primary/5 px-3 py-2.5 text-sm text-foreground">
            ...
            <ScrabbleBoard
              ...
              onCellDrop={(row, col) => {
                if (draggedRackId) placeTile(row, col, draggedRackId);
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-center">
              <Button
                ...
                onClick={() => {
                  ...
                  if (!key) {
                    setFeedback("There are no pending tiles to undo.");
                    return;
                  }
                  const tile = pending[key];
                  setPending((current) => {
                  ...
                  setFeedback("Last pending placement undone.");
                }}
              >
              ...
              >
                <Shuffle className="h-4 w-4" /> Exchange
              </Button>
              <Button
                size="sm"
                onClick={() => {
              ...
              >
                <Sparkles className="h-4 w-4" /> AI hint
              </Button>
              <Button
                size="sm"
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {currentRack.map((tile) => (
                <button
                  ...
                  className={`relative grid h-14 w-12 place-items-center rounded-lg border text-xl font-extrabold shadow-sm ${selectedRackId === tile.id ? "-translate-y-1 border-violet-500 bg-violet-100 ring-2 ring-violet-200" : "border-amber-200 bg-tile text-slate-800 hover:-translate-y-0.5"}`}
                >
                  {tile.letter}
        ...
        <aside className="min-w-0 space-y-4">
          <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-3 grid gap-2">
              ...
              >
                <span className="grid h-6 w-6 place-items-center rounded bg-amber-100 text-xs font-bold text-amber-700">
                  {remaining}
                </span>{" "}
                Tile bag{" "}
              ...
              >
                <Lightbulb className="h-4 w-4 text-violet-600" /> AI suggestions{" "}
                <span className="ml-auto text-xs text-muted-foreground">
                  Top 5
                </span>
              </Button>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Select a move to inspect the board moment.
                </p>
              </div>
        ...
        >
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-violet-600" /> AI suggestions
            </SheetTitle>
            <SheetDescription>
              Top 5 moves for your current rack. Violet tiles are a preview
              only.
            </SheetDescription>
          </SheetHeader>
          <div className="mt-6 space-y-3">
            {suggestions.map((suggestion, index) => (
              <button
                ...
                className={`w-full rounded-xl border p-3 text-left ${index === 0 ? "border-violet-300 bg-violet-50" : "border-border hover:border-violet-200"}`}
              >
                <div className="flex items-center gap-3">
                  <span className="grid h-7 w-7 place-items-center rounded bg-violet-600 text-xs font-bold text-white">
                    {index + 1}
                  </span>
          ...
          <Button
            ...
            onClick={() => {
              setPending((current) => ({ ...current, ...preview }));
              setPreview({});
              setAiOpen(false);
              setFeedback(
                "AI move staged as pending tiles. Submit to commit it.",
              );
            }}
          >
            Apply preview
      ...
      <AlertDialog open={passOpen} onOpenChange={setPassOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Passing ends your current turn. This local preview will keep the
              game open so you can continue testing.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                ...
                setPassOpen(false);
                setFeedback(
                  "You passed this turn. This local preview will keep the game open.",
                );
              }}
            >
              Pass turn
          ...
          {selectedMove && (
            <div className="space-y-5">
              ...
              <div className="rounded-xl bg-muted p-3 text-sm text-muted-foreground">
                Played {selectedMove.time}. The board position is shown in the
                move record for turn {selectedMove.turn}.
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <AlertDialog open={exchangeOpen} onOpenChange={setExchangeOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Select up to 7 tiles to return to the bag. Choose at least one
              tile to enable the exchange.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="flex flex-wrap justify-center gap-2">
            {currentRack.map((tile) => (
              <button
                ...
                className={`relative grid h-12 w-10 place-items-center rounded-lg border text-lg font-extrabold ${exchangeSelection.includes(tile.id) ? "border-violet-500 bg-violet-100 ring-2 ring-violet-200" : "border-amber-200 bg-tile"}`}
              >
                {tile.letter}
                <small className="absolute bottom-0.5 right-0.5 text-[8px]">
                  {tile.points}
                </small>
              </button>
            ))}
...
function ScoreCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 ${active ? "border-primary/30 bg-primary/5" : "border-border bg-card"}`}
    >
      <div className="flex items-center gap-3">
